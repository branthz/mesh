...
func (t *packetTransport) recvLoop() {
	defer log.Infoln("packet transport: recv loop exit")
	const maxRecvLen = 8192
	b := make([]byte, maxRecvLen)
	for {
		n, remote, err := t.conn.ReadFrom(b)
		if err != nil {
			log.Info("packet transport: recv: %v (aborting)", err)
			return
		} else if n >= cap(b) {
			log.Info("packet transport: recv from %s: short read, %d >= %d (continuing)", remote, n, cap(b))
			continue
		}
		var msg raftpb.Message
		if err := msg.Unmarshal(b[:n]); err != nil {
			log.Info("packet transport: recv from %s (sz %d): %v (%s) (continuing)", remote, n, err, b[:n])
			continue
		}
...
func (t *packetTransport) sendLoop() {
	defer log.Info("packet transport: send loop exit")
	for msg := range t.outgoingc {
		b, err := msg.Marshal()
		if err != nil {
			log.Info("packet transport: send to Raft ID %x: %v (continuing)", msg.To, err)
			continue
		}
		...
		if err != nil {
			...
			case t.unreachablec <- msg.To:
				log.Warn("packet transport: send to Raft ID %x: %v (unreachable; continuing) (%s)", msg.To, err, msg.Type)
			default:
				log.Warn("packet transport: send to Raft ID %x: %v (unreachable, report dropped; continuing) (%s)", msg.To, err, msg.Type)
			}
			continue
		}
		dst := meshconn.MeshAddr{PeerName: peerName}
		if n, err := t.conn.WriteTo(b, dst); err != nil {
			log.Info("packet transport: send to Mesh peer %s: %v (continuing)", dst, err)
			continue
		} else if n < len(b) {
			log.Info("packet transport: send to Mesh peer %s: short write, %d < %d (continuing)", dst, n, len(b))
			continue
		}
